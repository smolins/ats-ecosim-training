...
ecosim_ref="${ECOSIM_REF:-agraus/PrescribedPhenology}"
...
ecosim_commit="${ECOSIM_COMMIT:-$(git ls-remote https://github.com/jinyun1tang/EcoSIM.git "refs/heads/${ecosim_ref}" | awk '{print $1}')}"

if [[ -z "$amanzi_commit" || -z "$ats_commit" || -z "$ecosim_commit" ]]; then
  echo "Could not resolve Amanzi, ATS, or EcoSIM source revision" >&2
  exit 1
fi

echo "Amanzi: ${amanzi_commit}"
echo "ATS: ${ats_commit}"
echo "EcoSIM: ${ecosim_commit}"
...
docker build -f "${repo_root}/docker/Dockerfile.tpls" \
  ...
  --build-arg ECOSIM_COMMIT="$ecosim_commit" \
